...
@bp.post("/uploadfile")
@authenticated
async def uploadfile(auth_claims: dict[str, Any]):
    if "file" not in await request.files:
        return jsonify({"message": "No file part in the request", "status": "failed"}), 400

    file = (await request.files).getlist("file")[0]
    
    try:
        # Hardcoded Blob Storage URL with DefaultAzureCredential
        ...
        current_app.logger.error(f"Failed to upload file to Azure Blob Storage: {str(e)}")
        return jsonify({"message": "Failed to upload file", "status": "failed"}), 500
...
@authenticated
async def processdata(auth_claims: Dict[str, Any]):
    client = setup_azure_openai_client()
    assistant_file = upload_blob_to_azure(client)
    assistant = create_assistant(client, assistant_file)
    thread = client.beta.threads.create()

    content = "Give me all the insights and plots assuming this data is from an electric motor and give me graphs of rpm vs measures trq and all other useful possible plots to understand this motor characteristics"
    html_content = process_message(client, thread, assistant, content)

    return html_content

def setup_azure_openai_client() -> AzureOpenAI:
    return AzureOpenAI(
        api_key="",
...
def upload_blob_to_azure(client: AzureOpenAI) -> FileObject:
    BLOB_CONNECTION_STRING = "your_blob_connection_string"
    BLOB_CONTAINER_NAME = "data"
    BLOB_FILE_NAME = "Random_Data.csv"
    blob_url = f"/{BLOB_CONTAINER_NAME}/{BLOB_FILE_NAME}"

    return upload_file_from_blob(client, blob_url)

def upload_file_from_blob(client: AzureOpenAI, blob_url: str) -> FileObject:
    blob_client = BlobClient.from_blob_url(blob_url)
    download_file_path = Path(blob_url.split('/')[-1])

    with open(download_file_path, "wb") as download_file:
        download_file.write(blob_client.download_blob().readall())

    with download_file_path.open("rb") as f:
        file_object = client.files.create(file=f, purpose="assistants")
    
    download_file_path.unlink()
    return file_object

def create_assistant(client: AzureOpenAI, assistant_file: FileObject) -> Assistant:
    tools_list = [{"type": "code_interpreter"}]
    file_ids = [assistant_file.id]

    return client.beta.assistants.create(
        name="Electric Engine AI Assistant",
        instructions=(
            "You are an Electric Engine AI Assistant. "
            "Use the provided electric engine data CSV file to answer the questions. "
            "If the question is not related to engine data or you cannot answer the question, say, 'I do not know the answer.' "
            "If the user asks for help or says 'help', provide a list of sample questions that you can answer. "
            "Remember you have access to the uploaded file and a sandbox where you can run python code to help answer the questions."
        ),
        tools=tools_list,
...
def process_message(client: AzureOpenAI, thread: Thread, assistant: Assistant, content: str) -> str:
    ...
    return handle_run(client, thread, run)

def handle_run(client: AzureOpenAI, thread: Thread, run: Run) -> str:
    output_html = "<div>"
    ...
    while True:
        ...
        if run.status in ["completed", "failed", "expired", "cancelled"]:
            ...
            output_html += format_messages(client, messages)
            if run.status == "expired":
                output_html += "<p>The process expired before completion.</p>"
            elif run.status == "cancelled":
                output_html += "<p>The process was cancelled.</p>"
            break
...
def format_messages(client: AzureOpenAI, messages: Iterable[Message]) -> str:
    message_list = []
    html_output = ""
    ...
    for message in message_list:
        for item in message.content:
            if isinstance(item, TextContentBlock):
                html_output += f"<p><strong>{message.role.capitalize()}:</strong> {html.escape(item.text.value)}</p>"
                ...
                # Embed the image directly in the HTML using base64 encoding
                img_tag = f'<img src="data:image/png;base64,{img_str}" alt="Generated Chart" style="max-width: 100%;">'
                html_output += f"<p>{img_tag}</p>"
